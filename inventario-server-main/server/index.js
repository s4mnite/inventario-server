...
const cajaAbiertaEstadoQuery = {
  $or: [
    { estado: "abierta" },
    { abierta: true },
    { estado: { $exists: false }, cierre: { $in: [null, ""] } },
  ],
};
...
app.post("/api/caja/cerrar", async (req, res) => {
  try {
    ...
    if (caja) {
      const result = await db.collection("cajas").findOneAndUpdate(
        { _id: caja._id, ...cajaAbiertaEstadoQuery },
        { $set: cambios },
        { returnDocument: "after" }
      );
      cerrada = result?.value || result;
      if (!cerrada) return res.status(409).json({ error: "La caja ya estaba cerrada o cambió en otro dispositivo" });
    } else {
...
app.post("/api/ventas", async (req, res) => {
  try {
    ...
    const eggKey = empresaVenta || String(req.headers["x-usuario"] || "");
    let eggDoc = null;
    ...
    if (eggItems.length) {
      ...
      await db.collection("huevos").updateOne(
        { key: eggKey },
        { $set: { inventory: eggInventory, movements, usuario: req.headers["x-usuario"] || venta.usuario || "", empresa: empresaVenta, actualizadoEn: new Date() }, $setOnInsert: { creadoEn: new Date() } },
        { upsert: true }
      );
    }
