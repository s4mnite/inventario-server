...
export default function App() {
  ...
  const agregarProductoRapido = (prod) => {
    ...
    if (nuevaCantidad > Number(prod.stock || 0)) {
      setCarritoError(`Stock insuficiente para ${prod.nombre}.`);
      return;
    }
    const enPromo = prod.promoActiva && prod.promoCantMin && prod.promoPrecio && nuevaCantidad >= Number(prod.promoCantMin);
    const precio = enPromo ? Number(prod.promoPrecio) / Number(prod.promoCantMin) : Number(prod.precio || 0);
    if (existente) {
      setCarrito(prev => prev.map(c => String(c.productoId) === String(prod.id) && !c.esManga
        ? { ...c, cantidad: nuevaCantidad, precio, subtotal: precio * nuevaCantidad, enPromo }
        : c));
    } else {
      setCarrito(prev => [...prev, {
        productoId: prod.id, nombre: prod.nombre, img: prod.img, imagenUrl: prod.imagenUrl,
        precio, precioNormal: Number(prod.precio || 0), cantidad: 1, subtotal: precio,
        enPromo, promoLabel: enPromo ? `${prod.promoCantMin}x${fmt(Number(prod.promoPrecio))}` : null,
        esManga: false, mangaLabel: null, unidadesPorManga: 1, unidadesTotales: 1,
      }]);
    }
  };
              ...
              <section className="sales-mobile-v2">
                ...
                {(carrito.length>0 || freeEggItems.length>0) && <div className="sales-cart-v2">
                  ...
                  {carrito.map(item=><div className="sales-cart-row" key={`${item.productoId}-${item.esManga}`}>
                    ...
                    <div className="sales-cart-name"><b>{item.nombre}</b><small>{fmt(item.precio)} c/u</small></div>
            ...
            <div>
              ...
              <div style={card}>
                ...
                {boletas.length === 0 ? (
                  ...
                  <div>
                    {boletas.map(b => (
                      <div key={b.numero} className="receipt-list-card" style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 16px", background: bgCard2, borderRadius: 12, marginBottom: 10, border: `1px solid ${borderColor}`, cursor: "pointer" }} onClick={() => setBoletaModal(b)}>
                        <div className="receipt-list-icon" style={{ width: 42, height: 42, borderRadius: 12, background: D ? "rgba(215,25,32,0.15)" : "#fff3bf", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                          <Receipt size={18} color="#d71920" />
                        </div>
                        <div className="receipt-list-main" style={{ flex: 1, minWidth: 0 }}>
                          <div className="receipt-list-heading" style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                        ...
                        <div className="receipt-list-side" style={{ textAlign: "right", minWidth: 0 }}>
                          <p className="mono receipt-list-total" style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "#10b981" }}>{fmt(b.total)}</p>
                          <div className="receipt-list-actions" style={{ display: "flex", gap: 6, justifyContent: "flex-end", marginTop: 4, alignItems: "center" }}>
