...
import { API, fmt, todayLocalISO, calcIncrementPct, priceFromIncrement, fetchConTimeout } from "./lib/utils";
    ...
    {tab === "reportes" && <div className="egg-report-page" style={{display:"grid",gap:16}}>
      ...
      <div style={{...card,borderRadius:18,padding:16}}>
        ...
        <div className="egg-report-losses" style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:10}}>
          {[
            ["Roto",eggReport.totals.roto,"#d9483f"], ["Trizados",eggReport.totals.trizados,"#f3c122"], ["Total",eggReport.totals.totalPerdido,"#c7362f"]
      ...
      <div style={{...card,borderRadius:18,padding:0,overflow:"hidden"}}>
        ...
        <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:1350,fontSize:12}}>
          ...
          <tbody>{eggReport.categories.map(c=><tr key={c.id}><td style={{padding:11,borderBottom:`1px solid ${borderColor}`,fontWeight:800,color:textPrimary}}>{c.nombre}</td>{[c.stockInicial,c.entradas,fmt(c.valorEntradas),c.traspasos,c.vendidos,fmt(c.ingreso),fmt(c.costo),fmt(c.ganancia),c.roto,c.trizados,c.totalPerdido,fmt(c.valorPerdido),c.stockFinal].map((v,j)=><td key={j} style={{padding:11,borderBottom:`1px solid ${borderColor}`,color:j===7?(D?"#4fae93":"#18824b"):j===11?(D?"#d97757":"#c7362f"):textSecondary}}>{typeof v==="number"?v.toLocaleString("es-CL"):v}</td>)}</tr>)}
