...
const defaultEggInventory = [
  { id: "super", nombre: "Súper", stockHuevos: 0, costoCaja: 0, precioCaja: 0, precioBandeja: 0, precioVentaUnitario: 0, precioPromocionUnitario: 0, precioPromocionCaja: 0, precioPromocionBandeja: 0, precioEfectivoUnitario: 0, precioEfectivoCaja: 0, precioEfectivoBandeja: 0, precioEfectivoActivo: false, incrementoPct: 0, stockMinimoCajas: 5 },
  { id: "extra", nombre: "Extra", stockHuevos: 0, costoCaja: 0, precioCaja: 0, precioBandeja: 0, precioVentaUnitario: 0, precioPromocionUnitario: 0, precioPromocionCaja: 0, precioPromocionBandeja: 0, precioEfectivoUnitario: 0, precioEfectivoCaja: 0, precioEfectivoBandeja: 0, precioEfectivoActivo: false, incrementoPct: 0, stockMinimoCajas: 5 },
  { id: "primera", nombre: "Primera", stockHuevos: 0, costoCaja: 0, precioCaja: 0, precioBandeja: 0, precioVentaUnitario: 0, precioPromocionUnitario: 0, precioPromocionCaja: 0, precioPromocionBandeja: 0, precioEfectivoUnitario: 0, precioEfectivoCaja: 0, precioEfectivoBandeja: 0, precioEfectivoActivo: false, incrementoPct: 0, stockMinimoCajas: 5 },
  { id: "segunda", nombre: "Segunda", stockHuevos: 0, costoCaja: 0, precioCaja: 0, precioBandeja: 0, precioVentaUnitario: 0, precioPromocionUnitario: 0, precioPromocionCaja: 0, precioPromocionBandeja: 0, precioEfectivoUnitario: 0, precioEfectivoCaja: 0, precioEfectivoBandeja: 0, precioEfectivoActivo: false, incrementoPct: 0, stockMinimoCajas: 5 },
  { id: "tercera", nombre: "Tercera", stockHuevos: 0, costoCaja: 0, precioCaja: 0, precioBandeja: 0, precioVentaUnitario: 0, precioPromocionUnitario: 0, precioPromocionCaja: 0, precioPromocionBandeja: 0, precioEfectivoUnitario: 0, precioEfectivoCaja: 0, precioEfectivoBandeja: 0, precioEfectivoActivo: false, incrementoPct: 0, stockMinimoCajas: 5 },
];
...
export default function EggModule({ D, card, inp, textPrimary, textSecondary, textMuted, bgCard2, borderColor, borderColor2, currentUser, saleMode = false }) {
  ...
  // BUG FIX (carga lenta de Huevos): esta pestaña se desmonta por completo
  // cada vez que el usuario navega a otra sección (App.jsx solo la renderiza
  // cuando activeNav === "Huevos"), así que su estado se perdía y volvía a
  // arrancar en blanco cada vez que se volvía a entrar — de ahí el
  // "Cargando..." que se sentía lento aunque ya se hubiera cargado antes en
  // la misma sesión. Ya existían `loadEggInventory()`/`loadEggMovements()`
  // (leen la copia guardada en localStorage tras la última sincronización
  // exitosa) pero nunca se usaban para el estado inicial. Ahora la pestaña
  // se pinta al instante con la última copia conocida, y la sincronización
  // de abajo la refresca en segundo plano — igual que ya se hace con
  // ventas/productos en App.jsx.
  const [inventory, setInventory] = useState(loadEggInventory);
  const [movements, setMovements] = useState(loadEggMovements);
  ...
  const resetEggModule = async () => {
    ...
    const resetInv = defaultEggInventory.map(q => ({
      ...
      precioEfectivoUnitario: 0, precioEfectivoCaja: 0, precioEfectivoBandeja: 0,
    }));
  ...
  const saveQuality = async () => {
    ...
    const unitEfectivo = Math.max(0, Number(editForm.precioEfectivoUnitario || 0));
    ...
    const inventoryDelta = {
      ...
      precioEfectivoUnitario: unitEfectivo,
      precioEfectivoCaja: Math.round(unitEfectivo * EGG_BOX_UNITS),
      precioEfectivoBandeja: Math.round(unitEfectivo * EGG_TRAY_UNITS),
    ...
    {tab === "reportes" && <div className="egg-report-page" style={{display:"grid",gap:16}}>
      <div style={{...card,borderRadius:0,padding:16}}>
        ...
        <div className="egg-report-filters" style={{display:"grid",gridTemplateColumns:"minmax(170px,1fr) minmax(120px,180px)",gap:10,marginTop:15}}>
          <input type="date" value={reportDate} onChange={e=>setReportDate(e.target.value)} onClick={e=>{try{e.target.showPicker&&e.target.showPicker();}catch(err){}}} style={{...inp,cursor:"pointer"}}/>
