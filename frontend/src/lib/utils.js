...
export const computeEggLots = (movements, inventory) => {
      ...
      chronological.forEach(m => {
        ...
        if (m.tipo === "transferencia") {
          ...
          if (source && dest) {
            // Decisión del negocio: si el lote origen quedó con stock
            // NEGATIVO (se vendió/mermó más de lo que tenía anotado antes de
            // cerrarse), esa deuda se descarta al cerrar el lote — no se le
            // resta al lote nuevo. Solo se traspasa saldo cuando es positivo.
            const taken = Math.max(0, Number(source.stockRestante || 0));
            if (taken > 0) {
      ...
      Object.values(porCalidad).forEach(grupo => {
        ...
        ordenado.slice(0, -1).forEach(viejo => {
          ...
          // Decisión del negocio: si un lote viejo queda con stock NEGATIVO
          // (se vendió/mermó más de lo que tenía anotado antes de cerrarse),
          // esa deuda se descarta al cerrarse — no se arrastra al lote
          // vigente. Solo se traspasa saldo cuando es positivo.
